...
namespace XkCms.WebForm.Editor
{
	public class FileBrowserConnector : FileWorkerBase
	{
		protected override void OnLoad(EventArgs e)
		{
			...
			XmlNode oConnectorNode = CreateBaseXml( oXML, sCommand, sCurrentFolder ) ;
            ...
            {
                XmlNode oFoldersNode = XmlUtil.AppendElement(oConnectorNode, "Folders");
                XmlNode ow = XmlUtil.AppendElement(oFoldersNode, "NoLogin");
                XmlUtil.SetAttribute(ow, "name", "nologin");
            }
		...
		private XmlNode CreateBaseXml( XmlDocument xml, string command, string currentFolder )
		{
		...
		private void GetFiles( XmlNode connectorNode, string resourceType, string currentFolder )
		{
            ...
            {
                string ext = aFiles[i].Extension;
                if (ext.Length > 0) ext = ext.Substring(1);
                if (!CheckType(resourceType, ext)) continue;
        ...
        private bool CheckType(string resourceType, string ext)
        {
            if (resourceType == "Photo")
            {
                if ("jpg|jpe|jpeg|gif|bmp|png|ico|psd".IndexOf(ext) > -1)
                    return true;
                else
                    return false;
            }
